...
#define DONT_DEFINE_MALLOC
/* #define REGEX_MALLOC */
#define ORIG_MALLOC Nlm_MemNew
#define ORIG_FREE Nlm_Free
#define ORIG_REALLOC Nlm_Realloc
#endif /* REGEX_NCBI */
...
#define REGEX_ALLOCATE ORIG_MALLOC
#define REGEX_REALLOCATE(source, osize, nsize) ORIG_REALLOC (source, nsize)
...
#ifdef _WIN32 /* SAM CHANGE */
#define alloca _alloca

/* There are a million signed/unsigned warnings in this file.
 * Since it is not my code and I don't feel like fixing them,
...
#define TALLOC(n, t) ((t *) ORIG_MALLOC ((n) * sizeof (t)))
#define RETALLOC(addr, n, t) ((addr) = (t *) ORIG_REALLOC (addr, (n) * sizeof (t)))
...
NLM_EXTERN reg_syntax_t re_syntax_options = RE_SYNTAX_EMACS;
...
NLM_EXTERN reg_syntax_t re_set_syntax(reg_syntax_t syntax)
{
...
#define EXTEND_BUFFER()							\
  do {									\
    ...
    bufp->buffer = (unsigned char *) ORIG_REALLOC (bufp->buffer, bufp->allocated);\
    if (bufp->buffer == NULL)						\
      return REG_ESPACE;						\
...
NLM_EXTERN int re_compile_fastmap (struct re_pattern_buffer *bufp)
{
  int j, k;
...
NLM_EXTERN void re_set_registers (struct re_pattern_buffer *bufp,
...
NLM_EXTERN int re_search (struct re_pattern_buffer *bufp, const char *string,
...
NLM_EXTERN int re_search_2 (struct re_pattern_buffer *bufp,
...
#ifdef REGEX_MALLOC
#define FREE_VAR(var) if (var) ORIG_FREE (var); var = NULL
...
NLM_EXTERN int re_match (struct re_pattern_buffer *bufp,
...
NLM_EXTERN int re_match_2 (struct re_pattern_buffer *bufp,
			   const char *string1, int size1,
			   const char *string2, int size2,
...
static int
bcmp_translate (const char *s1, const char *s2, int len, char *translate)
{
...
NLM_EXTERN const char *re_compile_pattern (const char *pattern, int length,
					   struct re_pattern_buffer *bufp)
{
  reg_errcode_t ret;
...
NLM_EXTERN char *re_comp (const char *s)
{
  reg_errcode_t ret;
  ...
  if (!re_comp_buf.buffer)
    {
      re_comp_buf.buffer = (unsigned char *) ORIG_MALLOC (200);
      if (re_comp_buf.buffer == NULL)
	return "Memory exhausted";
      re_comp_buf.allocated = 200;

      re_comp_buf.fastmap = (char *) ORIG_MALLOC (1 << BYTEWIDTH);
...
NLM_EXTERN int re_exec (const char *s)
{
...
NLM_EXTERN int regcomp (regex_t *preg, const char *pattern, int cflags)
{
  reg_errcode_t ret;
  unsigned syntax
    = (cflags & REG_EXTENDED) ?
  ...
  if (cflags & REG_ICASE)
    {
      ...
      preg->translate = (char *) ORIG_MALLOC (CHAR_SET_SIZE);
...
NLM_EXTERN int regexec (const regex_t *preg, const char *string,
...
NLM_EXTERN size_t regerror (int errcode, const regex_t *preg,
...
NLM_EXTERN void regfree (regex_t *preg)
{
  if (preg->buffer != NULL)
    ORIG_FREE (preg->buffer);
  preg->buffer = NULL;
  ...
  if (preg->fastmap != NULL)
    ORIG_FREE (preg->fastmap);
  preg->fastmap = NULL;
  preg->fastmap_accurate = 0;

  if (preg->translate != NULL)
    ORIG_FREE (preg->translate);
  preg->translate = NULL;
}
