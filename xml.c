...
static struct connection *new_comic()
{
	...
	if(!(comics = realloc(comics, (n_comics + 1) * sizeof(struct connection)))) {
...
static void add_regmatch(struct connection **conn, xmlChar *regmatch)
{
	...
	if(*conn == NULL) *conn = new_comic();
	if((match = strtol((char*)regmatch, 0, 0)) >= MATCH_DEPTH)
...
static void parse_comic(xmlDocPtr doc, xmlNodePtr cur)
{
	...
	for(cur = cur->xmlChildrenNode; cur; cur = cur->next) {
		...
		if(strcmp(name, "url") == 0)
			add_url(&new, xmlNodeListGetString(doc, children, 1));
		else if(strcmp(name, "days") == 0)
			add_days(&new, xmlNodeListGetString(doc, children, 1));
		else if(strcmp(name, "regexp") == 0)
			add_regexp(&new, xmlNodeListGetString(doc, children, 1));
		else if(strcmp(name, "regmatch") == 0)
			add_regmatch(&new, xmlNodeListGetString(doc, children, 1));
		else if(strcmp(name, "output") == 0)
			add_outname(&new, xmlNodeListGetString(doc, children, 1));
		else if(strcmp(name, "href") == 0)
			add_base_href(&new, xmlNodeListGetString(doc, children, 1));
		else if(strcmp(name, "referer") == 0)
			add_referer(&new, xmlNodeListGetString(doc, children, 1));
		else if(strcmp(name, "optional") == 0)
			add_optional(&new, xmlNodeListGetString(doc, children, 1));
		else
...
int read_config(char *fname)
{
	...
	if(run_m4) {
		...
		if((out = mkstemp(tmpname)) < 0) {
			my_perror("mkstemp");
			return 1;
		}

		sprintf(line, "m4 %.200s", fname);
		if((in = popen(line, "r")) == NULL) {
			my_perror(line);
			return 1;
		}
	...
	for (cur = cur->xmlChildrenNode; cur; cur = cur->next) {
		...
		if(strcmp(name, "comic") == 0)
			...
			if(comics_dir) {
				if(verbose) printf("Ignore directory entry in XML file\n");
			}
			else
				comics_dir =
					must_strdup(str);
			...
			if(threads_set) {
				if(verbose) printf("Ignore threads entry in XML file\n");
			}
			else
	...
	if(randomize) randomize_comics();

	if(run_m4) unlink(tmpname);
	return 0;
...
inline
#endif
int write_tag(char *tag, char *value)
{
	if(value) fprintf(wfp, "  <%s>%s</%s>\n", tag, value, tag);
	return 0;
}
...
int write_comic(struct connection *conn)
{
	...
	if(first_time) {
		...
		fputs("<comics:Configuration xmlns:comics=\"http://seanm.ca/\">\n\n", wfp);
		...
		if(randomize) fputs("<randomize>1</randomize>\n", wfp);
		fputc('\n', wfp);
	...
	// bitmask not needed?
	if(conn->optional) fputs("  <optional>1</optional>\n", wfp);
	fputs("</comic>\n", wfp);
